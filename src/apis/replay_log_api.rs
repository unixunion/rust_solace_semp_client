...
pub struct ReplayLogApiClient<C: hyper::client::Connect> {
    configuration: Rc<configuration::Configuration<C>>,
}

impl<C: hyper::client::Connect> ReplayLogApiClient<C> {
    pub fn new(configuration: Rc<configuration::Configuration<C>>) -> ReplayLogApiClient<C> {
        ReplayLogApiClient {
...
impl<C: hyper::client::Connect>ReplayLogApi for ReplayLogApiClient<C> {
    fn get_msg_vpn_replay_logs(&self, msg_vpn_name: &str, count: i32, cursor: &str, _where: Vec<String>, select: Vec<String>) -> Box<Future<Item = ::models::MsgVpnReplayLogsResponse, Error = Error<serde_json::Value>>> {
