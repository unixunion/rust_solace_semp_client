...
pub struct TopicEndpointApiClient<C: hyper::client::Connect> {
    configuration: Rc<configuration::Configuration<C>>,
}

impl<C: hyper::client::Connect> TopicEndpointApiClient<C> {
    pub fn new(configuration: Rc<configuration::Configuration<C>>) -> TopicEndpointApiClient<C> {
        TopicEndpointApiClient {
            configuration: configuration,
        }
    }
}

pub trait TopicEndpointApi {
...
impl<C: hyper::client::Connect>TopicEndpointApi for TopicEndpointApiClient<C> {
    fn get_msg_vpn_topic_endpoints(&self, msg_vpn_name: &str, count: i32, cursor: &str, _where: Vec<String>, select: Vec<String>) -> Box<Future<Item = ::models::MsgVpnTopicEndpointsResponse, Error = Error<serde_json::Value>>> {
