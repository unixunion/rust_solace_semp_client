...
pub struct ClientProfileApiClient<C: hyper::client::Connect> {
    configuration: Rc<configuration::Configuration<C>>,
}

impl<C: hyper::client::Connect> ClientProfileApiClient<C> {
    pub fn new(configuration: Rc<configuration::Configuration<C>>) -> ClientProfileApiClient<C> {
        ClientProfileApiClient {
            configuration: configuration,
        }
    }
}

pub trait ClientProfileApi {
...
impl<C: hyper::client::Connect>ClientProfileApi for ClientProfileApiClient<C> {
    fn get_msg_vpn_client_profiles(&self, msg_vpn_name: &str, count: i32, cursor: &str, _where: Vec<String>, select: Vec<String>) -> Box<Future<Item = ::models::MsgVpnClientProfilesResponse, Error = Error<serde_json::Value>>> {
