...
pub struct MsgVpnQueue {
  /// The access type for delivering messages to client consumer flows bound to the Queue. The default value is `\"exclusive\"`. The allowed values and their meaning are:  <pre> \"exclusive\" - Exclusive delivery of messages to the first bound consumer flow. \"non-exclusive\" - Non-exclusive delivery of messages to all bound consumer flows in a round-robin fashion. </pre> 
  ...
  owner: Option<String>,
  /// The permission level for all client users of the Queue, excluding the owner. The default value is `\"no-access\"`. The allowed values and their meaning are:  <pre> \"no-access\" - Disallows all access. \"read-only\" - Read-only access to the messages in the Queue. \"consume\" - Consume (read and remove) messages in the Queue. \"modify-topic\" - Consume messages or modify the topic/selector of the Queue. \"delete\" - Consume messages, modify the topic/selector or delete the client created Queue altogether. </pre> 
  ...
  /// Enable or disable the return of negative acknowledgements (NACKs) to sending clients on message discards. Note that NACKs cause the message to not be delivered to any destination and transacted-session commits to fail. The default value is `\"when-queue-enabled\"`. The allowed values and their meaning are:  <pre> \"always\" - Always return a negative acknowledgment (NACK) to the sending client on message discard. \"when-queue-enabled\" - Only return a negative acknowledgment (NACK) to the sending client on message discard when the Queue is enabled. \"never\" - Never return a negative acknowledgment (NACK) to the sending client on message discard. </pre> 
