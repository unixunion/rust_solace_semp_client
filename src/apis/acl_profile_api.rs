...
pub struct AclProfileApiClient<C: hyper::client::Connect> {
    configuration: Rc<configuration::Configuration<C>>,
}

impl<C: hyper::client::Connect> AclProfileApiClient<C> {
    pub fn new(configuration: Rc<configuration::Configuration<C>>) -> AclProfileApiClient<C> {
        AclProfileApiClient {
...
impl<C: hyper::client::Connect>AclProfileApi for AclProfileApiClient<C> {
    fn get_msg_vpn_acl_profiles(&self, msg_vpn_name: &str, count: i32, cursor: &str, _where: Vec<String>, select: Vec<String>) -> Box<Future<Item = ::models::MsgVpnAclProfilesResponse, Error = Error<serde_json::Value>>> {
